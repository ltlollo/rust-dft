...
impl MathConsts for f64 {
    #[inline(always)]
    fn two_pi() -> f64 {
        6.28318530717958647692528676655900576f64
    }
    #[inline(always)]
    fn from_usize(val: usize) -> f64 {
        val as f64
    }
}
...
impl MathConsts for f32 {
    #[inline(always)]
    fn two_pi() -> f32 {
        6.28318530717958647692528676655900576f32
    }
    #[inline(always)]
    fn from_usize(val: usize) -> f32 {
        val as f32
    }
}
    ...
    fn preallocate(&mut self, siglen: usize) {
        if siglen / 2 > self.table.len() + 1 {
            for j in self.table.len()..siglen / 2 + 1 {
                let w: Vec<Complex<T>> = (0..j)
                                             .map(|x| {
                                                 let th = T::two_pi() * T::from_usize(x) /
                                                          T::from_usize(j);
...
pub fn fhwt<T>(sig: &mut [T]) -> Option<()>
    where T: MathConsts
{
    if !is_pow2(sig.len()) {
        return None;
...
/// the signal size must be a power of 2
pub fn fihwt<T>(sig: &mut [T]) -> Option<()>
    where T: Float
{
    if !is_pow2(sig.len()) {
        return None;
